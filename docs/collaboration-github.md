# Syncing and collaborating using GitHub {#github}
...
In [the previous lesson](#git), we learned how to manage your own code using version control. This is probably the bulk of what you will you do with version control. However, many of the benefits of using version control magnify when collaborating with others on code. For example:
...
* A section of code usually depends on what comes before it, so it isn't usually feasible for collaborators to write code chunks separately and combine them later, as you might for a document.

These tools are also useful even if you aren't working with other people on code. For example, using GitHub can help you:

* Sync repositories and changes across your personal devices
* Share code and data associated with a publication or report
* Host websites and other materials (like this book!)

Much rest of this lesson is copied, with minimal editing to images and links, from materials for [WILD 6900: Tools for Reproducible Science](https://ecorepsci.github.io/reproducible-science/), Spring 2021, Utah State University, by Dr. Simona Picardi, published under a [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/) license. 

## Adding a remote repository (sending your code to GitHub)
...
2. Create the remote first and “clone it” on your computer to get a local copy (see below).
...
<img src="images/ghdesktop_publishrepo.png" alt="Merge conflict in this R Markdown document" width="80%" />
<p class="caption">(\#fig:unnamed-chunk-1)Merge conflict in this R Markdown document</p>
</div>

## Cloning (adding a GitHub repository to your computer)

The other way to get a local-remote repository pair set up is to *clone* a GitHub repository to your computer. You could do this before starting a project, with an empty repository, or to collaborate on a shared repository. You and your collaborator/s can all have a copy of the same repository on each of your computers by cloning a shared remote repository, which will then be synchronized with changes made by anyone on the team. The person who created the repository will need to add the others as collaborators so that they’re able to clone it.

### Using GitHub Desktop

This is where we use the "Clone repository" option from the last lesson.

<img src="images/ghdesktop_newrepo.png" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

Following the prompts, you can add repositories from your personal GitHub account or from a URL.

## Fetching, pulling, and pushing commits

Now, we have a copy of a repository on our computer and one online. However, commits you make locally will not automatically appear on GitHub, and changes on GitHub will not automatically sync to your computer. Why? Well, imagine you and a colleague are working on the same repository at the same time. Changes they make, even if not to the same file, might impact your workflow in ways you aren't ready for. By choosing when to sync, you keep control of your repository. Here, we have three main verbs:

* **Fetching**: Look for changes on GitHub that haven't get been synced locally
* **Pulling**: Syncing changes on GitHub to your local repository
* **Pushing**: Synching changes to your local repository to GitHub

Again, GitHub Desktop makes these steps easy. At the top of your GitHub Desktop window, the far-right button will tell you what step you are at. First, fetch changes:

<img src="images/ghdesktop_fetch.png" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

If there are changes on GitHub that aren't reflected in your local respository, you can now pull them:

<img src="images/ghdesktop_pull.png" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

And once you've made a commit locally, you will have the option to push it to GitHub:

<img src="images/ghdesktop_push.png" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

The numbers reflect the number of commits that have been made since you last pushed or pulled.
...
<img src="images/ghdesktop_conflict.png" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

This is telling me that I have a merge conflict in my `Readme.md` file. If I open the file in RStudio, as suggested, I see this:

<img src="images/ghdesktop_resolve.png" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

Here, 

* `<<<<<<< HEAD`: Marks the beginning of the changes in my current local version.
* `=======`: Separates my changes from the incoming changes.
* `>>>>>>> [branch name]`: Marks the end of the incoming changes.

So, in this case, I added "in Natural Resources" to my local version, and I changed the course number in the incoming version. All I need to do is create the version I want, remove the `<<<<<<< HEAD`, `=======`, and `>>>>>>> [branch name]`, and save the file. Then, I can click "Continue Merge."

It is up to us to reconcile these changes however we consider appropriate. We could keep our own changes, keep the changes made by our collaborator, write something new to replace both, or delete the change entirely. Once the conflict is removed and the file is saved, we can proceed as usual by staging the file, committing it, and pushing it. Now, when any of the other collaborators pulls from the repository again, they will get the merged version of this file.

This takes some work, especially when changes are bigger than just a couple of words!
...
To create a new branch in GitHub Desktop, we use the middle panel in the menu:

<img src="https://docs.github.com/assets/cb-26427/mw-1440/images/help/desktop/new-branch-button-mac.webp" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

Or, if we already committed a change but want to make that commit into a new branch, we can use the History menu:

<img src="https://docs.github.com/assets/cb-141477/mw-1440/images/help/desktop/create-branch-from-commit-context-menu.webp" alt="" width="90%" style="display: block; margin: auto auto auto 0;" />

Branches follow the same processes for committing, publishing, fetching, pushing, and pulling as does the main branch. You can also bring your changes back and forth between branches. For more, see [the GitHub Desktop help page](https://docs.github.com/en/desktop/making-changes-in-a-branch/managing-branches-in-github-desktop).
...
<!-- ![](images/github_new_respository.png) -->
...
<!-- For example, if I forked octocat's Spoon-Knife repository onto my GitHub account, I could then clone it onto my computer like so: -->
...
<!-- Now I will be able to pull from the original repository and keep my local copy synchronized. -->
