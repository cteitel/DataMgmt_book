...
This lesson draws heavily from materials from WILD 6900: Tools for Reproducible Science, Spring 2021, Utah State University, by Dr. Simona Picardi, also published under a [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/) license and available at https://ecorepsci.github.io/reproducible-science/.
...
Error messages can be frustrating but they can be an important ally in diagnosing problems in your code. Some error messages (the good ones) are clear and point you to the problem quite explicitly. Others are cryptic, but they are so typical of  certain situations that they end up working as pretty good hints of what is wrong. The worst kind of error is the undescriptive, rare one that gives you no real clue where the problem is. Yoiu will encounter all of these. 
...
```{r debugging, fig.cap="Artwork by Allison Horst, and thanks to Simona Picardi for including this in her course materials", fig.align='center', out.width='80%', echo = FALSE, eval = TRUE}
